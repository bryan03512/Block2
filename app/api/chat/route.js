...
// Next.js 16 can evaluate top-level process.env reads at build time instead
// of request time unless gated behind a runtime API like connection() - so
// the client is built per-request, after connection(), not at module scope.
...
async function requestStream(client, messages, withThinkingDisabled) {
...
export async function POST(req) {
  ...
  try {
    ...
    completionStream = await requestStream(client, messages, true);
  } catch (error) {
    if (error?.status === 400 || error?.status === 422) {
      completionStream = await requestStream(client, messages, false);
    } else {
